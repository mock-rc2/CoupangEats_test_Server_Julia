...
public class StoreController {
 ...
 return new BaseResponse<>(getStoreMenuRes);
    ...
    /**
     * 주문내역 조회 API
     * [GET] /stores/order/:orderIdx
     * @return BaseResponse<GetOrderRes>
     */
    @ResponseBody
    @GetMapping("/order/{orderIdx}") // (GET) 127.0.0.1:9000/app/stores/order/:orderIdx
    public BaseResponse<GetOrderRes> getOrder(@PathVariable("orderIdx") int orderIdx) {
        try {
            GetOrderRes getOrderRes = storeProvider.getOrder(orderIdx);
     ...
     */
    @ResponseBody
    @GetMapping("/coupons") // (GET) 127.0.0.1:9000/app/stores/coupons
    public BaseResponse<List<GetCouponRes>> getCoupon() {
        try {
            List<GetCouponRes> getCouponRes = storeProvider.getCoupon();
